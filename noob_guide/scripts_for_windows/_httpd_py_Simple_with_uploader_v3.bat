...
echo To test this server, visiting http://localhost:8000 or http://127.0.0.1:8000
echo.
echo Listen address is 0.0.0.0:8000 by default, which should be :
echo ----- http://[This_Machines_IP_Address]:8000
echo.  
echo [Current_Working_Directory] is :
echo ----- [ %cd% ]
echo.
...
rem for /l %%a in (0,0,15) do timeout 10 >nul
rem taskkill /fi "windowtitle eq running-SimplePyHTTPd-uniTitle"
...
#rem # Easy Solution : change download path
#rem # right_click default downloads, and change path to sub-folder
#rem # Other Solution : make a windows softlink 
#rem # mklink /J "target_path" "original_path"
#rem ##############################################################
#rem ##############################################################
#rem # change dir to (pwd) 
#rem # cd %cd%
#rem # change dir to (win-user-default-download) 
#rem # cd %systemdrive%%homepath%\downloads 
#rem # cd %cd%
#rem # python ^%cd^%_py_SimpleHttpd_with_uploader.py
#rem # pause
#rem # exit
#rem ##############################################################